...
#for(lang in colnames(txt)) {
  ...
  jobsy.delta$col <- as.character(cut(jobsy.delta$pc.change * 100, breaks = c(-Inf, -4, 0, 4, Inf), labels = c("#ab3d3f", "#d68f90" , "#66b2b2", "#336666")))
  #jobsy.delta$col <- ifelse(jobsy.delta$pc.change > 0, "#336666", "#ab3d3f")
  #jobsy.delta$col <- ifelse(abs(jobsy.delta$pc.change > 4), jobsy.delta$col , alpha(jobsy.delta$col, 0.6))
  jobsy.delta$job.type <- names(job.type)[match(jobsy.delta$profession, job.type)]
  
  # sort job.type by pc.change
  job.type.sorted <- jobsy.delta %>% arrange(pc.change) %>% select(job.type) %>% unlist()
  
  plots <- lapply(job.type.sorted, function(p) {
    pc.delta <- jobsy.delta[which(jobsy.delta$job.type == p),'pc.change'] * 100
    ...
    mjs_plot(data = filter(dd, job.type == p), 
             ...
             left = 27, right = 10, bottom = 18, buffer = 0, top = 0, linked = F, 
             title = title) %>%
      mjs_line(area = TRUE, 
