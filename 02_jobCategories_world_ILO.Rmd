...
coutries.subsetl <- c('CHE', 'ESP', 'GBR', 'BRA', 'RUS', 'TUR', 'ITA', 'USA', 'FRA', 'JPN')
...
#### Use aggregated occupation

```{r mineagg}
agg <- data %>% filter(Classif1_Version_Label == "Aggregate categories", 
  Country_Code %in% c('FRA', 'DEU', 'ITA', 'ESP', 'CHE', 'GBR'))
...
agg <- agg %>% filter(Survey_Label == "EU Labour Force Survey")
# drop columns
agg <- agg %>% select(-Unit_Measure_Code, -Unit_Measure_Label, -Unit_Multiplier_Code, -Currency_Code, 
    -Currency_Label, -Notes_Source_Code, -Survey_Code, -Notes_Source_Code, -Notes_Indicator_Code, -Notes_Indicator_Label)

agg %>% group_by(Country_Label, Country_Code, Time) %>% summarise(length = length(Obs_Value)) %>% 
  ungroup() %>% as.data.frame()

agg %>% filter(Country_Label == "Switzerland", Time == 2009)


total.present <- agg %>% group_by(Country_Label, Time) %>% summarise(isTotal = any(Classif1_Item_Label == "Total")) %>% 
  ungroup() %>% select(isTotal) %>% unlist() %>% all()
if(!total.present) stop("Total occupation is not present for every year and country!")

# reshape to have TOTAL as an extra column
agg <- agg %>%  group_by(Country_Label, Time) %>% mutate(total = max(Obs_Value)) %>% ungroup()

ddd <- agg %>% filter(Classif1_Item_Label != "Total") %>% group_by(Country_Label, Time) %>% mutate(share = Obs_Value / total) %>% 
  ungroup() %>% select (-Unit_Multiplier_Label, -Source_Code, -Source_Label, -Survey_Label) 

ddd %>% #filter(!Classif1_Item_Label %in% c("Armed forces", "Not elsewhere classified")) %>% 
  ggplot(aes(Time, share)) + geom_line(aes(colour = Country_Label, 
    group = Country_Label), size = 0.7, alpha = 0.8) + ylab("% occupation") +
  facet_wrap(~ Classif1_Item_Label, ncol = 3, scales = "free_x") + swi_theme()

change <- ddd %>% group_by(Country_Label, Classif1_Item_Label) %>% 
  summarise(pc.change = ((dplyr::last(Obs_Value) - dplyr::first(Obs_Value)) /  dplyr::first(Obs_Value)) * 100)

change <-change %>% left_join(ddd %>% group_by(Country_Label, Classif1_Item_Label) %>% 
  summarise(delta = (dplyr::last(share) - dplyr::first(share)) * 100 ))

change <- change %>% left_join(ddd %>% group_by(Country_Label, Classif1_Item_Label) %>% 
  summarise(timeStart =  dplyr::first(Time) )) 

dw.change <- change %>% ungroup() %>% 
  filter(!Classif1_Item_Label %in% c("Armed forces", "Not elsewhere classified")) %>%
  select(-timeStart, -pc.change)
#spread
dw.change <- dw.change %>% spread(Country_Label, delta)

write.csv(dw.change, "input/EU_aggregateOccupation_shareDelta.csv", row.names = F)


```
#### Combine ISCO-08 & ISCO-88
...
```
** Big mess of a graphic! ** 
 
#### Use only ISCO-08
 
```{r mine2}
isc.08 <- data %>% filter(Classif1_Version_Label == "ISCO-08", Country_Code %in% coutries.subsetl)
...
isc.88 <- data %>% filter(Classif1_Version_Label == "ISCO-88", Country_Code %in% coutries.subsetl)
...
isc.08 <- isc.08 %>% select(-Unit_Measure_Code, -Unit_Measure_Label, -Unit_Multiplier_Code, -Currency_Code, 
    -Currency_Label, -Notes_Source_Code, -Survey_Code, -Notes_Source_Code, -Notes_Indicator_Code, -Notes_Indicator_Label)

# drop army jobs & unknown & non-Labour force survey
isc.08 <- isc.08 %>% filter(Source_Label == "Labour force survey") %>% filter(!Classif1_Version_Item_Code %in% c("OCU_ISCO08_0", "OCU_ISCO08_X"))

# check
isc.08 %>% group_by(Country_Label, Time) %>% summarise(length = length(Obs_Value)) %>% ungroup() %>% as.data.frame()


# compute the share of each occupation out of the total
total.present <- isc.08 %>% group_by(Country_Label, Time) %>% summarise(isTotal = any(Classif1_Version_Item_Code == "OCU_ISCO08_TOTAL")) %>% 
  ungroup() %>% select(isTotal) %>% unlist() %>% all()
if(!total.present) stop("TOTAL occupation is not present for every year and country!")

# reshape to have TOTAL as an extra column
isc.08 <- isc.08 %>%  group_by(Country_Label, Time) %>% mutate(total = max(Obs_Value)) %>% ungroup()

dd <- isc.08 %>% filter(Classif1_Version_Item_Code != "OCU_ISCO08_TOTAL") %>% group_by(Country_Label, Time) %>% mutate(share = Obs_Value / total) %>% 
  ungroup() %>% select (-Unit_Multiplier_Label, -Source_Code, -Source_Label, -Survey_Label) 

ggplot(dd, aes(Time, share)) + geom_line(aes(colour = Country_Label, group = Country_Label), size = 1, alpha = 0.8) + ylab("% occupation") +
  facet_wrap(~ Classif1_Version_Item_Code, ncol = 3, scales = "free_x") + swi_theme()
